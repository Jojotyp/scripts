...
# Pull last N screenshots from an Android phone via adb.
# Default N=1.
# Destination: ~/Screenshots/<phone_name>_<adb_serial>/
#
# usage examples:
# ~/Programming/scripts/screenshot-phone-sync/sync_screenshots.sh --n 50
#
# multiple phones connected
# adb devices
# ~/Programming/scripts/screenshot-phone-sync/sync_screenshots.sh --serial <ADB_ID> --n 30
...
# Candidate screenshot directories (Samsung typically uses Pictures/Screenshots)
candidates=(
...
if [ -z "${remote_dir}" ]; then
  echo "Could not find a screenshots directory on the device. Tried:" >&2
  printf '  %s\n' "${candidates[@]}" >&2
  exit 1
fi

# Get last N files by mtime (newest first).
# Using ls -t; if there are fewer than N files, we just get all of them.
# (Screenshots are usually png/jpg; we filter common extensions to avoid random files.)
files="$("${adb_base[@]}" shell "ls -1t \"$remote_dir\" 2>/dev/null | sed 's/\r$//' | grep -Ei '\.(png|jpe?g|webp)$' || true")"

if [ -z "${files}" ]; then
  echo "No screenshots found in: ${remote_dir}"
  exit 0
fi

# Limit to N (if N is larger than available, head just returns all)
# Use a temp file list to avoid subshell quoting surprises
tmp_list="$(mktemp)"
...
while IFS= read -r f; do
  [ -n "${f}" ] || continue
  # No spaces typically, but keep quoting anyway
