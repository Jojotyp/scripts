...
- `sync_camera.sh`: pulls the last N camera images, newest first. Defaults to N=1. It looks for camera images in common Android camera folders such as `/sdcard/DCIM/Camera`, `/sdcard/DCIM/100ANDRO`, and `/sdcard/Pictures/Camera`, then saves into `~/Pictures/phone/<phone_name>_<adb_serial>/camera/`.
