...
- `sync_recent.sh`: pulls the last N recent image and video files, newest first. Defaults to N=1. It checks recent entries in `/sdcard/DCIM`, `/sdcard/Pictures`, and WhatsApp Images, then saves into `~/Pictures/phone/<phone_name>_<adb_serial>/recent/`. Use `--whatsapp` to only check WhatsApp Images.
